...
import "../verifier/DocumentProofModal.css"; // Import CSS for the modal
...
const CitizenDocumentProofModal = ({ documentProofId, onClose }) => {
    ...
    const fetchDocument = async () => {
      ...
      try {
        const response = await authenticatedFetch(
          `/documents/proofs/${documentProofId}/view`,
          { method: "GET" }
        );
  ...
  const onDocumentLoadError = (pdfError) => {
    ...
    setError(
      `Failed to load PDF file. Details: ${pdfError.message || "Unknown error"}`
    );
    setNumPages(null);
    setPageNumber(1);
...
export default CitizenDocumentProofModal;
