...
const MyApplications = () => {
    ...
    useEffect(() => {
        ...
        const fetchApplications = async () => {
            try {
                ...
                const data = await authenticatedFetch(`/documents/applications?page=${currentPage}`, {
                    method: 'GET',
                });
                ...
                setError(err.message || 'Failed to load your applications. Please try again.');
            } finally {
                setLoading(false);
            }
        };

        if (token) { // Only fetch if token is available (user is logged in)
            fetchApplications();
        }
    ...
    if (applications.length === 0 && totalElements === 0) {
        return (
            <div className="container mt-5 text-center">
                ...
                <Link to="/citizen/submit-application" className="btn btn-primary rounded-pill px-4 py-2">
    ...
    return (
        <div className="container mt-5">
            <h2 className="text-center mb-4 text-primary fw-bold">My Submitted Applications</h2>
                            ...
                            <tr key={app.id}>
                                ...
                                <td>
                                    ...
                                    <button
                                        ...
                                        onClick={() => alert(`View details for Application ID: ${app.id}`)}
