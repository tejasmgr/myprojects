// --- src/pages/verifier/ApprovedApplications.js ---
...
const ApprovedApplications = () => {
    ...
    useEffect(() => {
        ...
        const fetchApprovedApplications = async () => {
            try {
                setLoading(true);
                setError('');
                const data = await authenticatedFetch(`/verifier/applications/approved/${user.id}?page=${currentPage}`, {
                    method: 'GET',
                });
                setApprovedApplications(data.content);
                setTotalPages(data.totalPages);
                setCurrentPage(data.number);
                setTotalElements(data.totalElements);
            } catch (err) {
                console.error('Failed to fetch approved applications:', err);
                setError(err.message || 'Failed to load approved applications. Please try again.');
            } finally {
                setLoading(false);
            }
        };
        ...
        return (
            <div className="d-flex justify-content-center align-items-center" style={{ height: '80vh' }}>
                <div className="spinner-border text-primary" role="status">
                    <span className="visually-hidden">Loading approved applications...</span>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="container mt-5 text-center">
                <div className="alert alert-info rounded-3" role="alert">
                    No approved applications at the moment.
                </div>
                <button className="btn btn-primary rounded-pill px-4 py-2" onClick={() => navigate('/dashboard')}>
                    Go to Dashboard
                </button>
    ...
    return (
        <div className="container mt-5">
            <h2 className="text-center mb-4 text-success fw-bold">Approved Applications</h2>
            ...
            {totalElements > 0 && (
                <p className="text-center mt-2 text-muted">
                    Showing {approvedApplications.length} of {totalElements} applications.
                </p>
            )}
        </div>
    );
